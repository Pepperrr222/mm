...
def solve_rolling(price: np.ndarray, load_s: np.ndarray, pv_s: np.ndarray, e_start: float,
    ...
    for s in range(S):
        ...
        for t in range(H):
            ...
            add([(h0+s*H+t,1),(y0+s*H+t,-mh)],hi=0)
            add([(c0+t,1),(y0+s*H+t,Q_MAX)],hi=Q_MAX)
            add([(w0+s*H+t,1),(y0+s*H+t,mw)],hi=mw)

    A = coo_matrix((vals,(rows,cols)), shape=(len(lows),nv)).tocsr()
    started = time.perf_counter()
    integrality=np.zeros(nv,dtype=np.uint8)
    integrality[z0:z0+H]=1; integrality[y0:y0+S*H]=1
    res = milp(objective, integrality=integrality,
               bounds=Bounds(lower,upper), constraints=LinearConstraint(A,np.asarray(lows),np.asarray(highs)),
...
def run_policy(data,pv_book,name,horizon,first_day,last_day,scenarios,time_limit,solve_mode="main"):
    ...
    for day in range(first_day,last_day+1):
        ...
        for k in range(0,N,UPDATE):
            ...
            if k==0:
                baseline=plan["grid"][:N].copy(); effective[:]=baseline
                update_cost=float(np.sum(data.price*baseline)); base_cost=update_cost
            else:
                effective[k:]=plan["grid"][:commit_len]
