"""问题4-2波动电价调度：每日0:00决策，比较24 h与48 h预测窗口。
...
PRICE4_PATH = ROOT / "01_题目与数据" / "原始附件" / "附件4.xlsx"
PRICE4 = None

def read_price4():
    x = pd.read_excel(PRICE4_PATH, sheet_name=0, header=None)
    a = x.iloc[1:, 1:].apply(pd.to_numeric, errors="coerce").to_numpy(float)
    dates = pd.to_datetime(x.iloc[1:, 0])
    if a.shape != (365, 144) or dates.isna().any() or np.isnan(a).any() or (a <= 0).any():
        raise RuntimeError("附件4价格表结构、日期或数值校验失败")
    if not dates.is_monotonic_increasing:
        raise RuntimeError("附件4日期不是递增序列")
    return a
...
def find_q2_module() -> Path:
    ...
    for path in CODE_DIR.glob("*.py"):
        if path == Path(__file__) or path.name.startswith(("problem3_", "problem4_")):
            continue
...
UPDATE = 144  # Question 4-2: one day-ahead decision at 0:00 only
...
def run_policy(data,pv_book,name,horizon,first_day,last_day,scenarios,time_limit,solve_mode="main"):
    ckpt_dir=ROOT/"99_临时文件"/"问题4_q42_checkpoints"
    ...
    for day in range(start,last_day+1):
        ...
        for k in range(0,N,UPDATE):
            ...
            # Causal day-ahead price forecast: last week's same weekday/slot.
            # Crossing midnight uses the corresponding historical next day.
            prices=np.asarray([PRICE4[max(0, day + (k+t)//N - 7), (k+t)%N]
                               for t in range(horizon)],float)
            commit_len=N-k
            ...
            if k==0:
                ...
                update_cost=float(np.sum(PRICE4[day]*baseline)); base_cost=update_cost
            else:
                effective[k:]=np.maximum(plan["grid"][:commit_len],0.0)
                up_cost=float(np.sum(PRICE4[day,k:]*UP_MULTIPLIER*plan["up"]))
                down_refund=float(np.sum(PRICE4[day,k:]*DOWN_REFUND*plan["down"]))
            ...
            for j in range(min(UPDATE,N-k)):
                ...
                day_rows.append({"date":data.dates[day],"policy":name,"t":t,"issue_slot":k,
                                 "price_yuan_per_kwh":PRICE4[day,t],"load_kw":data.load_kw[day,t],
                                 ...
                                 "emergency_cost_yuan":EMERGENCY_MULTIPLIER*PRICE4[day,t]*h})
                e=en
...
def main():
    ...
    ap.add_argument("--output-dir",default=str(ROOT/"04_结果"/"问题4_2_正式求解"))
    args=ap.parse_args(); out=Path(args.output_dir); out.mkdir(parents=True,exist_ok=True)
    global PRICE4
    data=q2.read_inputs(); PRICE4=read_price4(); pv_book=read_attachment3(); started=time.perf_counter()
    ...
    detail.to_csv(out/"问题4_2_逐时段结果.csv",index=False,encoding="utf-8-sig")
    daily.to_csv(out/"问题4_2_每日指标.csv",index=False,encoding="utf-8-sig")
    updates.to_csv(out/"问题4_2_更新级验证.csv",index=False,encoding="utf-8-sig")
    pivot.to_csv(out/"问题4_2_每日配对差.csv",index=False,encoding="utf-8-sig")
    summary.to_csv(out/"问题4_2_两方案汇总.csv",index=False,encoding="utf-8-sig")
    specified_table.to_csv(out/"问题4_2_指定日期结果.csv",index=False,encoding="utf-8-sig")
    (out/"问题4_2_总体验证.json").write_text(json.dumps(validation,ensure_ascii=False,indent=2),encoding="utf-8")
    elapsed=time.perf_counter()-started
    outputs=[p for p in out.iterdir() if p.is_file() and p.name!="问题4_2_复现清单.json"]
    manifest={"command":f"python 03_代码/problem4_q42.py --start-day {args.start_day} --end-day {args.end_day} --scenarios {args.scenarios} --solver-time-limit {args.solver_time_limit} --output-dir {args.output_dir}",
      ...
      "parameters":{"horizons_h":[24,48],"execute_h":24,"scenarios":args.scenarios,
      "eta_charge":ETA_C,"eta_discharge":ETA_D,"storage_bounds_kwh":[E_MIN,E_MAX]},
      "inputs":{str(q2.INPUT1):sha256(q2.INPUT1),str(q2.INPUT2):sha256(q2.INPUT2),str(ATTACHMENT3):sha256(ATTACHMENT3),str(PRICE4_PATH):sha256(PRICE4_PATH),str(Q2_PATH):sha256(Q2_PATH)},
    ...
    (out/"问题4_2_复现清单.json").write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding="utf-8")
