"""问题4-3波动电价调度：每6小时滚动更新，比较24 h与48 h预测窗口。
...
def run_policy(data,pv_book,name,horizon,first_day,last_day,scenarios,time_limit,solve_mode="main"):
    ckpt_dir=ROOT/"99_临时文件"/"问题4_checkpoints"
...
def main():
    ...
    ap.add_argument("--output-dir",default=str(ROOT/"04_结果"/"问题4_3_正式求解"))
    ...
    detail.to_csv(out/"问题4_3_逐时段结果.csv",index=False,encoding="utf-8-sig")
    daily.to_csv(out/"问题4_3_每日指标.csv",index=False,encoding="utf-8-sig")
    updates.to_csv(out/"问题4_3_更新级验证.csv",index=False,encoding="utf-8-sig")
    pivot.to_csv(out/"问题4_3_每日配对差.csv",index=False,encoding="utf-8-sig")
    summary.to_csv(out/"问题4_3_两方案汇总.csv",index=False,encoding="utf-8-sig")
    specified_table.to_csv(out/"问题4_3_指定日期结果.csv",index=False,encoding="utf-8-sig")
    (out/"问题4_3_总体验证.json").write_text(json.dumps(validation,ensure_ascii=False,indent=2),encoding="utf-8")
    elapsed=time.perf_counter()-started
    outputs=[p for p in out.iterdir() if p.is_file() and p.name!="问题4_3_复现清单.json"]
    manifest={"command":f"python 03_代码/problem4_full.py --start-day {args.start_day} --end-day {args.end_day} --scenarios {args.scenarios} --solver-time-limit {args.solver_time_limit} --output-dir {args.output_dir}",
    ...
    (out/"问题4_3_复现清单.json").write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding="utf-8")
